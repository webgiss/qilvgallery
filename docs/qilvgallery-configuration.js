...
jQuery.extend({
	...
	// A crude way of dete.ng if an object is a window
	...
	parseJSON: function( data ) {
		...
		// Make sure the inc. data is actual JSON
	...
	trim: trim ?
		...
		// Otherwise use our own tri. functionality
		function( text ) {
			return text == null ?
				"" :
...
(function() {
	...
	jQuery(function() {
		...
		// dete.ng if an element has been hidden directly using
...
jQuery.extend({
	...
	// A method for dete.ng if a DOM node can handle the data expando
...
var withinElement = function( event ) {
	...
	// ass. we've left the element since we most likely mousedover a xul element
	} catch(e) { }
},
...
// Perform a simple check to dete. if the browser is capable of
...
jQuery.fn.extend({
	...
	// Dete. the position of an element within
...
jQuery.extend({
	...
	// Dete.s if an XMLHttpRequest was successful or not
	...
	// Dete.s if an XMLHttpRequest returns NotModified
...
(function() {
  ...
  return VK = window.VK = {
    ...
    auto_bind: function(target, element) {
      ...
      binding_to_delete = [];
      $.each(this.global_bindings, (function(_this) {
        return function(keyvalue, targetmethod) {
      ...
      for (i = 0, len = binding_to_delete.length; i < len; i++) {
        keyvalue = binding_to_delete[i];
        delete this.global_bindings[keyvalue];
      }
      $.each(target.key_bindings, (function(_this) {
        return function(keyname, methodname) {
          _this.global_bindings[_this.getValue(keyname)] = [target, methodname];
      ...
      $.each(this.keys, (function(_this) {
        return function(index, keyname) {
          ...
          if (methodname != null) {
            ...
            if (methodname.substring(0, prefix.length) === prefix) {
              ...
              if (methodname === "") {
                delete _this.global_bindings[_this.getValue(keyname)];
              } else {
                _this.global_bindings[_this.getValue(keyname)] = [target, methodname];
              }
            }
...
// Generated by CoffeeScript 1.12.4
(function() {
  var $, GalleryOverlays, ImageOverlay, QILVGallery_overlays, VK, _QILVGallery_overlays, makeClass, window;
  ...
  ImageOverlay = makeClass({
    ...
    hide: function() {
      if (this.transition_time > 0) {
        ...
        this.img.fadeOut(this.transition_time, (function(_this) {
          return function() {
            _this.div.css("display", "none");
            return _this.img.css("z-index", 50000);
          };
        })(this));
      } else {
  ...
  GalleryOverlays = makeClass({
    ...
    set_transition_time: function(transition_time, silent) {
      ...
      $.each(['prev', 'next', 'current'], (function(_this) {
        return function(index, element) {
          _this[element].set_transition_time(transition_time);
          return true;
        };
      })(this));
      if (!silent) {
        ...
        return function(index, element) {
          if (_this[element]) {
            if (is_center_on_screen) {
              _this[element].centeronscreen();
            } else {
              _this[element].uncenteronscreen();
            }
          }
          return true;
        };
    ...
    prepare_next_slide: function() {
      return setTimeout((function(_this) {
        return function() {
          return _this.show_slideshow_slide();
        };
    ...
    go_prev: function() {
      ...
      this.current.hide();
      ref = [this.next, this.current, this.prev], this.prev = ref[0], this.next = ref[1], this.current = ref[2];
      this.current.show();
    ...
    go_next: function() {
      ...
      this.current.hide();
      ref = [this.prev, this.current, this.next], this.next = ref[0], this.prev = ref[1], this.current = ref[2];
      this.current.show();
    ...
    create_infotip: function(params) {
      ...
      if (params.fadeOut != null) {
        setTimeout((function(_this) {
          return function() {
            return $info_tip.fadeOut("slow", function() {
    ...
    toggle_infobox: function() {
      ...
      if ($('#QILVGallery_Infotip').length > 0) {
        ...
        text = "";
        $.each($(".QILVGallery_Image"), (function(_this) {
          return function(index, this_a) {
            if (_this.is_current(this_a.href)) {
              text += "<b>";
            }
            text += this_a.href;
            if (_this.is_current(this_a.href)) {
        ...
        $info_tip_pre.click((function(_this) {
          return function() {
            var $info_tip_area, count, max_len;
            text = "";
            count = 0;
            max_len = 0;
    ...
    update_preload_gauge: function() {
      ...
      if ($("#QILVGallery_preload_gauge").length > 0) {
        ...
        if (panel._loaded === panel._total) {
          return $("#QILVGallery_preload_gauge").fadeOut(2000, (function(_this) {
            return function() {
              return $(_this).remove();
            };
          })(this));
      ...
      if ($("#QILVGallery_preload_all_panel").length === 0) {
        ...
        $images.each((function(_this) {
          return function(index, this_a) {
            ...
            $img.load(function() {
              $panel[0]._loaded += 1;
              return _this.update_preload_gauge();
            });
            $panel.append($img);
            return true;
          };
    ...
    help: function() {
      ...
      if ($('#QILVGallery_Help').length > 0) {
        ...
        $.each(VK.global_bindings, (function(_this) {
          return function(vk_value, vk_props) {
            if (vk_props[0] === _this) {
              $info_tip_div.append($("<div/>").html("<b>" + VK.getName(vk_value) + "</b>: " + (vk_props[1] in _this.bindables ? _this.bindables[vk_props[1]] : vk_props[1])));
            }
            return true;
          };
        })(this));
        ...
        $info_tip_div = $("<div/>").css('margin-left', '10px');
        $.each(this.configurables, (function(_this) {
          return function(attr, comment) {
            $info_tip_div.append($("<div/>").html("<b>" + comment + " (current value)</b>: " + (GM_values['QILV.' + attr] || "default") + " (" + _this[attr] + ")"));
            return true;
          };
        })(this));
        $info_tip.append($info_tip_div);
    ...
    __init__: function() {},
    init: function() {
      var $link_list, element, index, index_element, j, len, previouslink, ref;
      window.test = {};
      window.test.ImageOverlay = ImageOverlay;
      window.test.makeClass = makeClass;
      ref = [[0, "prev"], [1, "current"], [2, "next"]];
      ...
      previouslink = void 0;
      $link_list = $("a").filter((function(_this) {
        return function(index, this_a) {
          var link, ok;
...
(function() {
  jQuery(function($) {
    var addkeysbuttons, addvaluesbuttons, prefix_url, ref, setlink, setlink_using_param, setlinkname, suffix_url;
    ref = [window.qilv_prefix_url, window.qilv_suffix_url], prefix_url = ref[0], suffix_url = ref[1];
    setlink = function(keyboard_config) {
      var first, keystring;
      keystring = "";
      ...
      $('#gallery_link').attr('href', "javascript:(function(){if(window.QILVGallery_overlays){window.QILVGallery_overlays.current.show();}else{" + keystring + "var n=document.createElement('script');n.type='text/javascript';n.src='" + prefix_url + "qilvgallery-total.js" + suffix_url + "';document.getElementsByTagName('head')[0].appendChild(n);}})();");
      $('#gallery_link').hide();
      return $('#gallery_link').fadeIn('slow');
    };
    setlinkname = function() {
      return $('#gallery_link').text($('#namefield').val());
    };
    setlink_using_param = function() {
      var config;
      config = {};
    ...
    addkeysbuttons = function(keyname, actionname) {
      var $combo, $line;
    ...
    addvaluesbuttons = function(name, value, comment) {
      var $combo, $line;
      $line = $("<tr><td class='key'></td><td class='action'></td></tr>");
